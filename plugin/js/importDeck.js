...
export function importDeck(event) {
  ...
  let displayError = (error, msg) => {
    ...
    // clear file input
    $('#importInput').replaceWith($('#importInput').val('').clone(true));

    let reply = `<div class='error'>${error}</div>`;
    if (msg) reply += `<div>${msg}</div>`;
    display(reply);
  }

  // check if deck already contains cards
  if ($("#total_cards_in_deck").text() != "0" ||
    $("input[type='checkbox'][name*='delete']").length > 0) {
    return displayError("Current deck must not contain cards");
  }
  ...
  openFile((contents, error) => {
    ...
    try {
      ...
      try {
        ...
        return displayError("Invalid File Configuration", error.message);
      }
    }

    checkDeckLegal(contents, (error) => {
      if (error) {
        return displayError("Illegal Deck", error);
      }
      display("Building deck, please wait");
      buildDeck(contents, (error) => {
        if (error) {
          return displayError("Deck Building Error", error);
        }
        else {
